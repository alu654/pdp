...
nivelDeDificultadDeLaSala(NombreSala,Dificultad):-
    ...
    tipoDeSala(Experiencia,Dificultad).

tipoDeSala(terrorifica(CantidadSusto, EdadMinima),Dificultad):-
    Dificultad is (CantidadSusto -EdadMinima).

tipoDeSala(familiar(futurista, _),15).
tipoDeSala(familiar(Tematica, CantidadDeHabitaciones),Dificultad):-
Tematica \=futurista,
    CantidadDeHabitaciones is Dificultad.

tipoDeSala(enigmatica(ListaCandados),Dificultad):-
...
/*puedeSalir/2: una persona puede salir de una sala si la dificultad de la sala es 1 o si tiene más de 13 años y 
la dificultad es menor a 5. En ambos casos la persona no debe ser claustrofóbica.*/
